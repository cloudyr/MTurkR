...
function (xml = NULL, xml.parsed = NULL) 
{
    ...
    for (i in 1:iterations) {
        z <- z + 1
        values$AssignmentId[i] <- xmlValue(xpathApply(answers[[j]], "../AssignmentId")[[1]])
        values$WorkerId[i] <- xmlValue(xpathApply(answers[[j]], "../WorkerId")[[1]])
        values$HITId[i] <- xmlValue(xpathApply(answers[[j]], "../HITId")[[1]])
