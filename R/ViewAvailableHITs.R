    ...
    qualified = NULL) 
{
    parameters <- ""
    if (!is.null(query)) 
        parameters <- paste(parameters, "searchWords=", query, 
            "&", sep = "")
    if (!is.null(requester)) 
        parameters <- paste(parameters, "requesterId=", requester, 
            "&", sep = "")
    if (!is.null(min.reward)) 
        parameters <- paste(parameters, "minReward=", min.reward, 
            "&", sep = "")
    if (!is.null(qualified) && (qualified == TRUE | qualified == 
        "true")) 
