    ...
    browser = FALSE, log.requests = TRUE, sandbox = FALSE) 
{
    ...
    if (!is.null(description)) 
        GETparameters <- paste(GETparameters, "&Description=", 
            curlEscape(description), sep = "")
    if (!is.null(status)) 
        GETparameters <- paste(GETparameters, "&QualificationTypeStatus=", 
            status, sep = "")
    if (!is.null(test)) 
    ...
    if (is.null(auto) & !is.null(auto.value)) 
        GETparameters <- paste(GETparameters, "&AutoGrantedValue=", 
            auto.value, sep = "")
